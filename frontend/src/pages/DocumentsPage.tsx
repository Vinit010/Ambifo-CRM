...
import { Download, FileArchive, FileDown, FileImage, FilePlus2, FileSpreadsheet, FileText, Film, FolderUp, Mail, Plus, Search, Trash2, X } from 'lucide-react'
import { customerApi, documentApi } from '../api'
import { Button, Badge, EmptyState, Field, inputCls, PageHead, Panel, selectCls, Spinner } from '../components/ui'
...
export default function DocumentsPage() {
  ...
  return (
    <div>
      <PageHead title="Documents" subtitle="Engine-generated SOWs and per-customer file storage" />
...
interface SowSection { title: string; body: string }
...
function SowTab() {
  ...
  const generateMutation = useMutation({
    mutationFn: () =>
      documentApi.sowGenerate({
        customer_id: Number(form.customer_id),
        sow_title: form.sow_title || undefined,
        sections: sections.filter((s) => s.body.trim()),
      }),
    onSuccess: (sow) => {
      queryClient.invalidateQueries({ queryKey: ['sows'] })
      setSelected(sow.id)
      setForm({ customer_id: '', sow_title: '' })
      setSections([{ title: 'Overview', body: '' }, { title: 'Scope of Work', body: '' }, { title: 'Commercials', body: '' }])
    },
  ...
  function addSection() {
    setSections((s) => [...s, { title: `Section ${s.length + 1}`, body: '' }])
  }
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
      <Panel className="scroll-fade lg:col-span-1">
        <h2 className="mb-4 flex items-center gap-2 font-display text-sm font-bold tracking-wide text-navy-900 uppercase">
          <FilePlus2 size={15} className="text-brand-teal" /> Generate SOW
        </h2>
        <form
        ...
        >
          <Field label="Customer *">
            <select className={selectCls} required value={form.customer_id} onChange={(e) => setForm((f) => ({ ...f, customer_id: e.target.value }))}>
          ...
          <Field label="SOW title">
            <input className={inputCls} value={form.sow_title} onChange={(e) => setForm((f) => ({ ...f, sow_title: e.target.value }))} placeholder="Cloud Migration SOW" />
          </Field>
            ...
            {sections.map((sec, idx) => (
              <div key={idx} className="rounded-xl border border-slate-200 bg-slate-50/50 p-3">
                <div className="mb-2 flex items-center justify-between">
                  <input className={`${inputCls} !border-0 !bg-transparent font-display text-xs font-bold`} value={sec.title} onChange={(e) => updateSection(idx, 'title', e.target.value)} placeholder="Section title" />
                  {sections.length > 1 && (
                    <button type="button" className="text-slate-400 hover:text-red-500" onClick={() => removeSection(idx)}><X size={14} /></button>
                  )}
                </div>
                <textarea className={`${inputCls} min-h-[60px] !text-xs`} value={sec.body} onChange={(e) => updateSection(idx, 'body', e.target.value)} placeholder="Section content…" />
              </div>
            ))}
          </div>

          <Button type="button" variant="outline" icon={<Plus size={14} />} onClick={addSection}>Add section</Button>

          {error && <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-600">⚠ {error}</div>}
          <Button disabled={generateMutation.isPending} icon={<FileDown size={15} />} type="submit">
            {generateMutation.isPending ? 'Generating…' : 'Generate via engine'}
          </Button>
        </form>
      </Panel>
      ...
      <Panel className="scroll-fade lg:col-span-2" style={{ animationDelay: '0.1s' }}>
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <h2 className="flex items-center gap-2 font-display text-sm font-bold tracking-wide text-navy-900 uppercase">
            <FileText size={15} className="text-brand-teal" /> SOW library
          </h2>
        ...
        {sows.isLoading ? (
          ...
          <div className="space-y-3">
                ...
                {selected === s.id && (
                  <div className="border-t border-slate-100 p-4">
                    <div className="mb-3 flex items-center gap-2">
                      ...
                      <Button variant="ghost" className="!px-3 !py-1.5 text-xs text-red-500" icon={<Trash2 size={13} />} onClick={() => { if (confirm('Delete this SOW?')) deleteMutation.mutate(s.id) }}>
                        Delete
                      </Button>
                    </div>
            ...
            {sows.data?.length === 0 && <EmptyState message="No SOWs generated yet" />}
          </div>
        )}
      </Panel>
    </div>
